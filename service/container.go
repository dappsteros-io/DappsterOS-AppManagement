...
import (
	...
	"github.com/dappsteros-io/DappsterOS-AppManagement/pkg/utils/envHelper"
	v1 "github.com/dappsteros-io/DappsterOS-AppManagement/service/v1"
...
func getContainerStats() {
	...
	for i := 0; i < 100; i++ {
		...
		if config.DappsterOSGlobalVariables.AppChange {
			config.DappsterOSGlobalVariables.AppChange = false
...
func (ds *dockerService) GetContainerAppList(name, image, state *string) (*[]model.MyAppList, *[]model.MyAppList) {
	...
	defer cli.Close()
	// fts := filters.NewArgs()
	// fts.Add("label", "dappsteros=dappsteros")
	// fts.Add("label", "dappsteros")
	// fts.Add("dappsteros", "dappsteros")
	containers, err := cli.ContainerList(context.Background(), types.ContainerListOptions{All: true})
	if err != nil {
		logger.Error("Failed to get container_list", zap.Any("err", err))
	}
	...
	dappsterOSApps := []model.MyAppList{}
	...
	for i, m := range containers {
		...
		if m.Labels["dappsteros"] == "dappsteros" {
			...
			if m.Labels["origin"] == "system" {
				...
				if len(strings.Split(name, "/")) > 1 {
					icon = "https://icon.dappsteros.io/main/all/" + strings.Split(name, "/")[1] + ".png"
				}
			}

			dappsterOSApp := model.MyAppList{
				Name:       name,
				Icon:       icon,
				State:      m.State,
			...
			dappsterOSApps = append(dappsterOSApps, dappsterOSApp)
		} else {
	...
	return &dappsterOSApps, &localApps
}
...
func (ds *dockerService) CreateContainer(m model.CustomizationPostData, id string) (containerID string, err error) {
	...
	if err == nil {
		...
		config = info.Config
		if config.Labels["dappsteros"] == "dappsteros" {
			config.Cmd = m.Cmd
	...
	config.Labels["dappsteros"] = "dappsteros"
